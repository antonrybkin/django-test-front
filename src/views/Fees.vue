  ...
  <v-container>
    ...
    >
      <template v-slot:item.id="{ item }">
        <router-link :to="{ name: 'Transactions', params: { id: item.id } }"
          >Счёт {{ item.id }}</router-link
        >
      </template>
      <template v-slot:item.actions="{ item }">
        <v-btn color="primary mr-3" @click="wantToPay(item.id)"
          >Пополнить</v-btn
        >
        <v-btn
          :disabled="item.balance === '0.00'"
          color="primary"
          class="mr-3"
          @click="wantToSpend(item.id)"
          >Потратить</v-btn
        >
    ...
    <div class="grey--text mt-3 mb-3">
      Колличество активных счетов: {{ this.$store.state.fees.length }}
    </div>
...
export default {
  ...
  computed: {
    pages() {
      return Math.ceil(
        this.$store.state.fees.length / this.pagination.itemsPerPage
      );
    }
  },
  methods: {
